...
function Modal({ product, onClose, onSave, allProducts }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (skuTaken) { setError('This SKU is already in use by another product'); return; }
...
export default function VendorProducts() {
  ...
  return (
    <div className="px-4 sm:px-6 py-6 sm:py-8 max-w-7xl mx-auto">
                ...
                {products.map((p) => (
                  <tr key={p.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => setModal(p)} className="text-xs text-blue-600 hover:underline font-medium">Edit</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {modal && (
        <Modal product={modal === 'add' ? null : modal} onClose={() => setModal(null)}
          onSave={handleSave} allProducts={products} />
      )}
    </div>
  );
}
