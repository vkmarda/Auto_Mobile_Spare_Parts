...
export default function VendorDashboard() {
  ...
  const alerts = [
    counts.pending > 0 && {
      key: 'pending', label: `⚠️ ${counts.pending} order${counts.pending > 1 ? 's' : ''} pending action`,
            ...
            {chartsLoading ? <div className="h-[260px]"><Spinner /></div>
              ...
              : (
                <ResponsiveContainer width="100%" height={260}>
                  <PieChart>
                    <Pie data={productStats} dataKey="total_qty" nameKey="name" cx="50%" cy="50%"
                    ...
                    <Tooltip content={({ active, payload }) => {
                      ...
                      return <div className="bg-white border border-gray-200 rounded-lg p-2 text-xs shadow-sm"><p className="font-semibold text-gray-800 mb-1">{d.name}</p><p className="text-gray-600">Qty: {d.total_qty} units</p></div>;
                    }} />
