...
const BRAND_COLORS = {
  ...
  'honda-sc':     'bg-red-50 border-red-300 text-red-700',
  'tvs-sc':       'bg-orange-50 border-orange-300 text-orange-700',
  'yamaha-sc':    'bg-blue-50 border-blue-300 text-blue-700',
  'suzuki-sc':    'bg-sky-50 border-sky-300 text-sky-700',
  'bajaj-sc':     'bg-indigo-50 border-indigo-300 text-indigo-700',
};
...
export default function BrandStep() {
  ...
  useEffect(() => {
    ...
    getBrands(vehicleType.id).then(setBrands).finally(() => setLoading(false));
  }, [vehicleType]);
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-2xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          {vehicleType && (
            <span className="bg-blue-100 text-blue-700 text-xs font-medium px-3 py-1 rounded-full">{vehicleType.name}</span>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {brands.filter(b => b.name.toLowerCase().includes(search.toLowerCase())).map((b) => {
              const colors = sel === b.id
                ? 'bg-blue-600 border-blue-600 text-white'
                : (BRAND_COLORS[b.slug] || 'bg-gray-50 border-gray-200 text-gray-700');
              return (
