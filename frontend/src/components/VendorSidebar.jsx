...
import { LayoutDashboard, Clock, ClipboardList, Truck, RotateCcw, Store, Package, Wrench } from 'lucide-react';

const NAV = [
  { to: '/vendor',           label: 'Dashboard', icon: LayoutDashboard, dot: null },
  { to: '/vendor/pending',   label: 'Pending',   icon: Clock,           dot: 'pending' },
  { to: '/vendor/orders',    label: 'Orders',    icon: ClipboardList,   dot: null },
...
function SidebarInner({ expanded, onToggle, onClose }) {
  ...
  const [counts, setCounts] = useState({ pending: 0, accepted: 0, returns: 0 });

  useEffect(() => {
    getAllOrders()
      .then((orders) => setCounts((c) => ({
        ...c,
        pending:  orders.filter((o) => o.status === 'pending').length,
        accepted: orders.filter((o) => o.status === 'accepted').length,
      }))).catch(() => {});
    getReturns()
      .then((rets) => setCounts((c) => ({
        ...c,
        returns: rets.filter((r) => r.status === 'return_requested').length,
      }))).catch(() => {});
  }, [pathname]);

  const getBadge = (dot) => {
    const count = counts[dot];
    if (!dot || !count) return null;
    return { count, cls: DOT_COLORS[dot] };
  };

  const isActive = (to) =>
    to === '/vendor'
      ? pathname === '/vendor' || pathname === '/vendor/dashboard'
        ...
        {NAV.map(({ to, label, icon: NavIcon, dot }) => {
          ...
          return (
            ...
            >
              ...
              {badge && !expanded && (
                <span className={`${badge.cls} absolute top-1 right-1 w-3.5 h-3.5 rounded-full border-2 border-white`} />
              )}
            </Link>
          );
        })}
          ...
          <>
            <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-indigo-400">Vendor</p>
              </div>
            </div>
            <button
...
export default function VendorSidebar({ expanded, onToggle, mobileOpen, onMobileClose }) {
  return (
    <>
      {/* Desktop sidebar */}
      <aside
